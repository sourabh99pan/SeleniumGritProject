...
public class TC_MPAdvanceMixerSalesManagerN_SD_T extends TestBase {
	...
	public void TC_09_MPAdvanceMixerSalesManagerN_SD_T(String Username, String Password, String LoginUser,
		...
		if (exists) {
			...
			SpecialDesign.click();
			test.pass("Special design checkbox selected");
		}

		WebElement VariancefromRSMThreshold = driver.findElement(By.xpath(
				"(//span[contains(text(),'Variance from Generated Discount')]//following::lightning-formatted-text)[1]"));
		String VarianceGeneratedDiscount = VariancefromRSMThreshold.getText();
		String ActualRSMNumber = VarianceGeneratedDiscount.replaceAll("[^.\\s0-9\\s]", "");

		double num2 = Math.round(Double.parseDouble(ActualRSMNumber) * 100.0) / 100.0;

		if (num2 == 0) {
			test.pass("Variance from Generated Discount equal to expected'" + VarianceGeneratedDiscount);
		} else {
			test.fail("Variance from Generated Discount not equal to expected'" + VarianceGeneratedDiscount);
		}

		WebElement OppoTerms = driver
				.findElement(By.xpath("(//span[contains(text(),'Terms')]//following::lightning-formatted-text)[1]"));
		String OppoTerms1 = OppoTerms.getText();

		if (OppoTerms1.equalsIgnoreCase(OpportunityTerms)) {
			test.pass("Opportunity terms match as expected'" + OppoTerms);
		} else {
			test.fail("Opportunity terms does not match as expected'" + OppoTerms);
		}
