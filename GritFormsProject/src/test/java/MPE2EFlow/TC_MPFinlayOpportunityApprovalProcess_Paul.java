...
public class TC_MPFinlayOpportunityApprovalProcess_Paul extends TestBase {
	...
	public void TC_24_MPFinlayOpportunityApprovalProcess_Paul(String Username, String Password, String LoginUser, String AccountStatus, String AccountHolder,
		...
		WebElement totalStdDiscount = driver
				.findElement(By.xpath("(//span[contains(text(),'Total Standard Discount Variance')]//following::lightning-formatted-text)[1]"));
		String StdDiscountVariance = totalStdDiscount.getText();
		String ActualstdDiscountVariance=StdDiscountVariance.replaceAll("[^.\\s0-9\\s]", "");
		String ExpectedDiscountVariance="60000";
		double num2 = Math.round(Double.parseDouble(ActualstdDiscountVariance) * 100.0) / 100.0;
		double num1= Math.round(Double.parseDouble(ExpectedDiscountVariance)* 100.0) / 100.0;
		if (num2>num1) {
			test.pass("Total Standard Discount Variance value greater then expected'" + totalStdDiscount);
		} else {
			test.fail("Total Standard Discount Variance value not greater then expected'" + totalStdDiscount);
		}
