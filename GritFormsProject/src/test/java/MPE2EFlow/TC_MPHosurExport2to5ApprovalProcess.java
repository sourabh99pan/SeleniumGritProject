...
public class TC_MPHosurExport2to5ApprovalProcess extends TestBase {
	...
	public void TC_39_MPHosurExport2to5ApprovalProcess(String Username, String Password, String LoginUser,
		...
		WebElement DiscountBelowConfig = driver.findElement(By.xpath(
				"(//span[contains(text(),'Discount below Config (%)')]//following::lightning-formatted-number)[1]"));
		...
		if (num2 > 0.02) {
			...
			test.fail(
					"Discount below config percent value not greater then expected'" + ActualstdbelowDiscountVariance);
		}

		WebElement TotalStandardDiscountVariancePercent = driver.findElement(By.xpath(
		...
		if (STDisVariance > 0.02) {
			...
			test.fail(
					"Total Standard Discount Variance % value not greater then expected'" + ActualstdDiscountVariance);
		}
