...
public class TC_MPLSVTWSOpptyApprovalProcess_General4 extends TestBase {
	...
	public void TC_49_MPLSVTWSOpptyApprovalProcess_General4(String Username, String Password, String LoginUser, String AccountStatus, String AccountHolder,
		...
		WebElement VariancefromRSMThreshold = driver
				.findElement(By.xpath("(//span[contains(text(),'Variance from RSM Threshold')]//following::lightning-formatted-text)[1]"));
		String RSMThreshold = VariancefromRSMThreshold.getText();
		String ActualRSMNumber=RSMThreshold.replaceAll("[^.\\s0-9\\s]", "");
		String ExpectedRSMthreshold="150000.00";
		...
		if (num2>num1) {
			test.pass("RSM threshold value greater then expected'" + RSMThreshold + "' successfully");
		} else {
			test.fail("RSM threshold value not greater then expected'" + RSMThreshold);
		}
