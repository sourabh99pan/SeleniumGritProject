...
public class TC_MPPowerscreenOpportunityAP_N extends TestBase {
	...
	public void TC_56_MPPowerscreenOpportunityAP_N(String Username, String Password, String LoginUser, String AccountStatus, String AccountHolder,
		...
		WebElement totalStandarddiscount = driver.findElement(By.xpath("(//span[contains(text(),'Total Standard Discount Variance')]//following::lightning-formatted-text)[1]"));
		String DiscountVariance = totalStandarddiscount.getText();
		String ActualDiscountVariance=DiscountVariance.replaceAll("[^.\\s0-9\\s]", "");
		String ExpectedDiscountVariance="60000.00";
		if (ActualDiscountVariance!="") {
			 num2= Math.round(Double.parseDouble(ActualDiscountVariance) * 100.0) / 100.0;
		} else {
			test.fail("Total Standard Discount value is Empty");
		}
		
		if (ExpectedDiscountVariance!="") {
			 num1= Math.round(Double.parseDouble(ExpectedDiscountVariance) * 100.0) / 100.0;
		} else {
			test.fail("Total Standard Discount value is Empty");
		}
		
		if (num2>num1) {
			test.pass("Total Standard discount variance value greater then expected'" + totalStandarddiscount);
		} else {
			test.fail("Total Standard discount variance value is not greater then expected'" + totalStandarddiscount);
		}
