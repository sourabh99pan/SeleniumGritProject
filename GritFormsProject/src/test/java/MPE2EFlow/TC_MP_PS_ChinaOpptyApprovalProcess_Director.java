...
public class TC_MP_PS_ChinaOpptyApprovalProcess_Director extends TestBase {
	...
	public void TC_04_MP_PS_ChinaOpptyApprovalProcess_Director(String Username, String Password, String LoginUser,
		...
		WebElement TotalStandardDiscountVariance = driver.findElement(By.xpath(
				"(//span[contains(text(),'Total Standard Discount Variance %')]//following::lightning-formatted-Number)[1]"));
		String DOAValue = TotalStandardDiscountVariance.getText();
		String TotalStandardDiscount = DOAValue.replaceAll("[^.\\s0-9\\s]", "");
		double num2 = Math.round(Double.parseDouble(TotalStandardDiscount) * 100.0) / 100.0;
		Thread.sleep(5000);
		if (num2 > 7) {
			test.pass("DOA percentage value greater then expected'" + DOAValue);
		} else {
			test.fail("DOA percentage value is not greater then expected'" + DOAValue);
		}
